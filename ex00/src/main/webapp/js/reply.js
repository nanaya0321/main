 ...
 */
 ...
 // 일반게시판 댓글을 처리하는 객체 선언
 // jquery의 ajax를 사용함 - ajax(), getJSON(), get(), post() 이런것들이 있음
 let replyService = {
 	...
 	"list": function(page, callback, error) {
 		...
 		// ajax 형태로 만들어 처리함 - getJSON() 으로 받음
 		$.getJSON( // (url, 성공함수, 실패함수)
 			...
 			function(data) {
 				...
 				// 이걸 사용하는 이유는 태그형태가 달려졌을때 사용하기 위해서다.
 		...
 		// 실패했을때 실행되는 함수
 			...
 			// error가 있으면 실행
 			if (error) error();
 			else alert("댓글 데이터 가져오는 중 오류 발생");
 		});
 	},
 	// 2. 일반게시판 댓글 등록 처리
 	...
 	"write": function(reply, callback, error) {
 		...
 		$.ajax({
 			...
 			data: JSON.stringify(reply), // 서버에 전송되는 데이터 - body에 전송됨
 			contentType: "application/json; charset:utf-8", // 서버에 전동되는 데이터 타입과 엔코딩 방식을 적는다.
 			// 성공했을때 함수
 	...
 	"update": function(reply, callback, error) {
 		...
 		$.ajax({
 			type: "post", // 데이터 전송 방식
 			url: "/boardreply/update.do", // RestController에 mapping되는 url을 적는다.
 			data: JSON.stringify(reply), // 서버에 전송되는 데이터 - body에 전송됨
 			contentType: "application/json; charset:utf-8", // 서버에 전동되는 데이터 타입과 엔코딩 방식을 적는다.
 			// 성공했을때 함수
 			success: function(result, status, xhr) {
