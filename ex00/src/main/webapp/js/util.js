 ...
 */
 ...
 "totalPage":1,"totalRow":6,"key":null,"word":null,"period":"pre","pageQuery":"page=1&perPageNum=10&key=&word=",
 "notPageQuery":"perPageNum=10&key=&word="}}
 */
 
 // 댓글 리스트 표시했던 것과 동일하게 댓글 페이지네이션 태그들의 문자열을 만들어 넘겨준다.
 function replyPagination(pageObject) {
	...
	// 이전페이지를 누르면 startPage -1 로 이동하도록 구현 (data-page에 세팅)
	str += '" data-page="' + (pageObject.startPage-1) + '"><a class="page-link" href="#">이전페이지</a></li>';
	// *** 앞 페이지 그룹으로 이동(Previous) - 끝
	
	// startPage 부터 endPage 까지 반복처리하면서 페이지 버튼을 만들기
	...
	// 다음 페이지 그룹으로 이동
	// 1-10, 11-20, 21-30 totalPage -> 마지막 페이지
	str += '<li class="page-item';
	if (pageObject.endPage >= pageObject.totalPage) {
		str += ' disabled ';
	}
	// 다음 페이지 버튼을 누르면 endPage + 1 로 이동 (data-page에 세팅)
	str += '" data-page="' + (pageObject.endPage + 1) + '"><a class="page-link" href="#">다음페이지</a></li>';
	return str;
