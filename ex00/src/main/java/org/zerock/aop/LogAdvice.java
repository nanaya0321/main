...
public class LogAdvice {
	...
	// servlet에서 execute() 함수에서 했던 처리를 대신 하도록 만듬
	// ProceedingJoinPoint - 실행객체, 넘어가는 데이터가 함께 담겨있음
	// - 실행해야할 객체 : ~~ServiceImpl
	// - 넘어가는 데이터(parameter - no, pageObject, vo...)
	// @Around()는 실행각체 양쪽에 표시
	// '*'는 모든 값을 의미하는 기호
	...
	public Object longTime(ProceedingJoinPoint pjp) throws Throwable {
		...
		// 실행되는 매서드 출력
		log.info("++ 실행매서드: " + pjp.getSignature());
		// 전달되는 파라메타 출력
		// pjp.getArgs() -> 배열로 되어있다.
		// 이때 배열을 문자열로 바꿔주는 것은 Arrays.toString(pjp.getArgs());
		log.info("++ 전달되는 데이터: " + Arrays.toString(pjp.getArgs()));
		log.info("++ ***********************************************************************");
