...
@Controller
// servlet-context.xml에 base-package가 정의되어있는지 확인
@RequestMapping("/sample/*")
@Log4j
public class SampleController {
	
	// Return이 없을시 (void type) 들어온 uri 이름으로 jsp경로가 생성됩니다.
	// Return이 String
	...
	// uri 매핑을 Get, Post를 허용하는것 아무것도 정하지 않았으면 기본 Get 방식
	@RequestMapping(value = "/basic", method = {RequestMethod.GET, RequestMethod.POST})
	// method가 하나만 있을때는 {} 생략 가능
	...
	// get방식 매핑
	@GetMapping("/ex02")
	// defaultValue에 적는 값은 반드시 String 타입으로 적어야 한다.
	public String ex02(String name, @RequestParam(defaultValue = "0") int age) {
	...
	// get방식 매핑
	@GetMapping("/ex03")
	public String ex03(@RequestParam("name") String name, @RequestParam(name = "age", defaultValue = "0") int age) {
		log.info("get name : " + name + ", age= " + age);
		return "ex03";
	}
	...
	// get방식 매핑
	@GetMapping("/ex04List")
	// List타입의 변수는 @RequestParam로 name을 정의해줘야한다.
...
//	}
	...
	public void exUpload() {
		log.info("exUpload---------------------------");
		// void type은 /WEB-INF/views/ + sample/exUpload + .jsp
	}
	// post 방식 매핑 - 파일 올리기 처리(저장은 제외)
